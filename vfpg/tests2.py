# -*- coding: utf-8 -*-
"""
Created on Tue Apr 19 10:13:33 2022
...
def path_gen(path_manifold,q_manifold,S_manifold,input_size,vfpg,q,L,t,m,w,int_weights):
    global q_params
...
def loss():
    ...
    global loss,loss_kl,loss_i,loss_f,lnz,error,I,S,f
    global S_mean,q_mean
    
    # We sample the paths, their probs and their actions 
    manager = mp.Manager()

    path_manifold = manager.list([])
    q_manifold = manager.list([])
    S_manifold = manager.list([])

    pool = mp.Pool(processes=4)       
    for _ in range(n_paths):
        pool.apply(path_gen,args=(path_manifold,q_manifold,S_manifold,input_size,vfpg,q,L_HO,t,m,w,int_weights,))
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    def pic_creator(bound,j):
        ...
        ax1.plot([e for e in range(j+1)],loss_kl_list,label='$\mathcal{L}_{KL}$')
